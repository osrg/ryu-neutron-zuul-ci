...
function setup_localrc() {
    ...
    DEFAULT_ENABLED_SERVICES=g-api,g-reg,key,n-api,n-crt,n-obj,n-cpu,n-sch,mysql,rabbit,sysstat,dstat,pidstat
    DEFAULT_ENABLED_SERVICES+=,cinder,c-api,c-vol,c-sch,n-cond
...
#        fi
        if [ "$DEVSTACK_GATE_NEUTRON" -eq "1" ]; then
            MY_ENABLED_SERVICES=$MY_ENABLED_SERVICES,neutron,q-svc,q-agt,q-dhcp,q-l3,q-meta,q-lbaas,q-fwaas
            if [ "$DEVSTACK_GATE_RYUPLUGIN" -eq "1" ]; then
                MY_ENABLED_SERVICES=$MY_ENABLED_SERVICES,ryu
            elif [ "$DEVSTACK_GATE_OFAGENT" -eq "1" ]; then
                MY_ENABLED_SERVICES=$MY_ENABLED_SERVICES,q-vpn
            else
                MY_ENABLED_SERVICES=$MY_ENABLED_SERVICES,q-vpn,q-metering
            fi
            echo "Q_USE_DEBUG_COMMAND=True" >>localrc
            echo "NETWORK_GATEWAY=10.1.0.1" >>localrc
        else
