...
function git_clone_and_cd {
    ...
    local server=${3:-https://git.openstack.org}
...
function setup_project {
    ...
    local server=https://git.openstack.org
    if [ ${project%%/*} = "osrg" ]; then
        server=https://github.com
    fi

    echo "Setting up $project @ $branch"
    git_clone_and_cd $project $short_project $server

    git_remote_set_url origin $server/$project
...
function setup_workspace {
    ...
    sudo find ~/cache/files/ -mindepth 1 -maxdepth 1 -exec cp {} $DEST/devstack/files/ \;
...
function cleanup_host {
    ...
    if [ -d $BASE/old ]; then
      ...
      sudo cp $BASE/old/tempest/etc/tempest.conf $BASE/logs/old/tempest.conf.txt
